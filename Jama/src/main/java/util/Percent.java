...
public final class Percent implements Comparable<Percent> {
	...
	public Percent add(Percent arg){
		return new Percent(this.value.add(arg.value));
	}
	
	public Percent addAll(Percent... args){
		return addAll(Arrays.asList(args));
	}
	
	public Percent addAll(Collection<Percent> args){
	...
	public Percent subtract(Percent arg){
		return new Percent(this.value.subtract(arg.value));
	}
